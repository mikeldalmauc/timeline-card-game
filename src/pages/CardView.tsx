...
import { isCardFlipped, markCardFlipped, getDrawnCards } from "../lib/store";
...
export default function CardView() {
  ...
  const handleThumbnailClick = (cardId: string) => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F3EF] flex font-serif">
      {/* SIDEBAR - Drawn Cards */}
      <div className="hidden md:flex flex-col w-32 bg-[#FAF8F5] border-r border-[#D1CABF] p-4 gap-3 overflow-y-auto">
        <p className="text-[10px] uppercase tracking-widest font-bold text-[#5A5A40] opacity-60">Cartas Mostradas</p>
        <div className="flex flex-col gap-2">
          {drawnCards.length === 0 ? (
            <p className="text-[10px] text-[#B5AE9E] italic">Sin cartas</p>
          ) : (
            drawnCards.map((drawnId) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-1">
                    <p className="text-[8px] text-white font-bold truncate">{drawnCard.title}</p>
                  </div>
                </motion.button>
              );
            })
